$(document).ready(function(){
    ...
    $('#save').click(function() {
        if ( $('#usrname').val().length == 0 || $('#spwd').val().length == 0 ) console.log('error:: username or password missing');
            ...
            // switch to chrome storage since there is need to share data between content script and background script and pop-ups
            const usernameKey = domain+'username';
            const passwordKey = domain+'password';

            let obj = {}
            obj[usernameKey] =$('#usrname').val();
            obj[passwordKey] = $('#spwd').val()

            chrome.storage.sync.set(obj, function() {
                // some operation after been saved
                alert("Saved")
            });
